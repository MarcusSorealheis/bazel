...
public final class RepositoryDelegatorFunction implements SkyFunction {
  ...
  public SkyValue compute(SkyKey skyKey, Environment env)
    ...
    if (isFetch.get()) {
      ...
      /* If this is NOT a local repository, try to retrieve it from cache using the marker file.
        Local repository are generally fast and do not rely on non-local data, making caching
        them across server instances impractical.*/
      if (!handler.isLocal(rule)) {
        boolean needsConfiguring = handler.isConfigure(rule) ?
        ...
        if (doNotFetchUnconditionally && !needsConfiguring && repoRoot.exists()) {
      ...
      /* Else(This is a force fetch, a local repository or the repo was not cached), Initiate the
      fetching process */
  ...
  @Nullable
  @Override
  public String extractTag(SkyKey skyKey) { return null; }
